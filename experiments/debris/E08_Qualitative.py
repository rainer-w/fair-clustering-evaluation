...
from src.utils.helpers import evaluate_groundtruth_separate, add_gt_deviations, summarize_gt_deviations
import pandas as pd
import numpy as np
...
def main_8(run=True,plot=True):
    ...
    if plot: 
        ...
        for i,fd in id_fde_dict.items():
            ...
            plot_filtered_skyline(stacked_map[i], x="n_clusters", path=f"results/debris/Experiment8/Setting_{i}_all_n_clusters_",xlim=0,ylim=0)
            gt_rows = evaluate_groundtruth_separate(fd)
