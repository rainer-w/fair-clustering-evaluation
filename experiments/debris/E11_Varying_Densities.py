...
def run_11(path,dim,clunum,seed,core_num,ratio_noise,g,distr, n, alpha):
    ...
    fairdata.to_csv(f"{path}_{alpha}_data.csv",index=False)
    ...
    stacked["dim"] = dim 
    stacked["alpha"] = alpha
    stacked["cluster_ratios"] = str(cluster_ratios)
    rows = []
    ...
    groundtruth_eval["alpha"] = alpha
    groundtruth_eval["cluster_ratios"] = cluster_ratios
    rows.append(groundtruth_eval)
    return pd.DataFrame(rows), stacked
def main_11(run=True,plot=True):
    ...
    DIM = 10
    N = 1000
    base_path = "results/debris/Experiment11/"
    if run:
        for seed in SEEDS:
            path = f"{base_path}{seed}/"
            Path(path).mkdir(parents=True,exist_ok=True)
            opt_dfs = []
            for i,alpha in enumerate([100,10,1,0.2,0.05]):
                opt_df, all_df = run_11(path,DIM,CLUNUM,seed,CORE_NUM,RATIO_NOISE,G,DISTR,N,alpha)
                opt_dfs.append(opt_df)
                all_df.to_csv(f"{path}alpha={alpha}.csv", index=False)
            ...
            if plot:
                plot_line(results=results, x="alpha", path=path, categorical=True)
    avg_results, gt_results = aggregate_over_seeds(
    base_path, seeds = SEEDS, group_cols= ["alpha", "method", "criterion"]
    )

    plot_line(avg_results, x="alpha", path=base_path, categorical=True, xlabel="alpha of dirichlet distr. for cluster density", groundtruth_results=gt_results, include_std=True)
