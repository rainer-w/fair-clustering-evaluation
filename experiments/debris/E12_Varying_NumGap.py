...
def run_12(path,dim,clunum,seed,core_num,ratio_noise,g,distr, n, gap):
    ...
    fairdata.to_csv(f"{path}_{gap}_data.csv",index=False)
...
def main_12(run=True,plot=True):
    ...
    DIM = 10
    N = 1000
    base_path = "results/debris/Experiment12/"
    if run:
        for seed in SEEDS:
            path = f"{base_path}{seed}/"
            Path(path).mkdir(parents=True,exist_ok=True)
            opt_dfs = []
            for i,gap in enumerate([0,10,20,30]):
                opt_df, all_df = run_12(path,DIM,CLUNUM,seed,CORE_NUM,RATIO_NOISE,G,DISTR,N,gap)
                opt_dfs.append(opt_df)
                all_df.to_csv(f"{path}gap={gap}.csv", index=False)
            ...
            if plot:
                plot_line(results=results, x="gap", path=path, categorical=True)
    avg_results, gt_results = aggregate_over_seeds(
    base_path, seeds = SEEDS, group_cols= ["gap", "method", "criterion"]
    )

    plot_line(avg_results, x="gap", path=base_path, categorical=True, xlabel="number of gap cores", groundtruth_results=gt_results, include_std=True)
