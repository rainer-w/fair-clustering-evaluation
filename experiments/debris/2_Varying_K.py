...
def run_2(path,dim,clunum,seed,core_num,ratio_noise,g,distr, n):
    ...
    fairdata.to_csv(f"{path}_{clunum}_data.csv",index=False)
...
def main_2(run=True,plot=True):
    ...
    if run:
        ...
        for seed in SEEDS: 
            ...
            opt_dfs = []
            for clunum in [2,4,6,10,16,20]:
                CORE_NUM = [CORES_PER_CLUSTER] * clunum
                opt_df, all_df = run_2(path,DIM,clunum,seed,CORE_NUM,RATIO_NOISE,G,DISTR,n=1000)
                opt_dfs.append(opt_df)
                all_df.to_csv(f"{path}k={clunum}.csv", index=False)
            ...
            # plot individual lines per seed
            if plot:
                plot_line(results, x="clunum", path=path)
   ...
   # print("GT RESULTS , ", gt_results)
    plot_line(avg_results, x="clunum", path=base_path, groundtruth_results=gt_results, include_std=True)
