...
def run_4(path,dim,clunum,seed,core_num,ratio_noise,g,distr, n):
    ...
    fairdata.to_csv(f"{path}_{dim}_data.csv",index=False)
...
def main_4(run=True,plot=True):
    ...
    base_path = "results/debris/Experiment4/"
    if run:
        for seed in SEEDS:
            path = f"{base_path}{seed}/"
            Path(path).mkdir(parents=True,exist_ok=True)
            opt_dfs = []
            for dim in [10,20,50,100,500,1000]:
                opt_df, all_df = run_4(path,dim,CLUNUM,seed,CORE_NUM,RATIO_NOISE,G,DISTR,n=1000)
                opt_dfs.append(opt_df)
                all_df.to_csv(f"{path}dim={dim}.csv", index=False)
    ...
    base_path, seeds = SEEDS, group_cols= ["dim", "method", "criterion"]
    )

    plot_line(avg_results, x="dim", path=base_path, categorical=False, groundtruth_results=gt_results, include_std=True, logaxis="x")
