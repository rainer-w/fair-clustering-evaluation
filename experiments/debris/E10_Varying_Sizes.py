...
def run_10(path,dim,clunum,seed,core_num,ratio_noise,g,distr, n, a):
    ...
    fairdata.to_csv(f"{path}_{a}_data.csv",index=False)
...
def main_10(run=True,plot=True):
    ...
    CLUNUM = 5
    AVG_CORES_PER_CLUSTER = 15
    DIM = 10
    N = 1000
    base_path = "results/debris/Experiment10/"
    if run:
        for seed in SEEDS:
            ...
            for a in range(0,13,2):
                CORE_NUM = [
                    AVG_CORES_PER_CLUSTER - a,
                    AVG_CORES_PER_CLUSTER - a,
                    AVG_CORES_PER_CLUSTER, 
                    AVG_CORES_PER_CLUSTER + a, 
                    AVG_CORES_PER_CLUSTER + a, 
                ]
                opt_df, all_df = run_10(path,DIM,CLUNUM,seed,CORE_NUM,RATIO_NOISE,G,DISTR,N,a)
                opt_dfs.append(opt_df)
                all_df.to_csv(f"{path}a={a}.csv", index=False)
