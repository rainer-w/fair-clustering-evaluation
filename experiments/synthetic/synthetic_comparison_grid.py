...
from src.generators.grid_generator import GridGenerator
...
def run_grid(path,seed,n):
    
    fairdegen = GridGenerator(grid_len=5)
...
def main_grid(run=True,plot=True):
    
    SEEDS = [11,22,33,44,55] 
    base_path = "results/synthetic/GridGenerator/"
    if run:
        for seed in SEEDS:
            ...
            for n in [500, 1000, 2000, 5000, 10000]:
                opt_df, all_df = run_grid(path,seed,n=n)
