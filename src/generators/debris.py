# DEBRIS is an extension of the DENSIRED generator for evaluating 
# fair density-based clustering under imbalance of sensitive attributes
# DENSIRED implementation by Philipp Jahn:
# https://github.com/PhilJahn/DENSIRED/tree/main
# DEBRIS by Rainer Wöß
...
def t_ball_num(center, radius, d, n, clunum, df, seed):
	...
	return x
class DEBRIS: 
	def __init__(self, dim, clunum, core_num, ratio_noise, seed, g, distr, gap =2, clu_ratios=None): 
		cluster_ratios = clu_ratios
		if cluster_ratios is None:
			cluster_ratios = np.ones(clunum) / clunum
		self.gen = densityDataGen(dim=dim, clunum=clunum, core_num=core_num, ratio_noise=ratio_noise, seed=seed, g=g, distr=distr, gap = gap, clu_ratios=cluster_ratios)
	def generate_data(self, data_num, center=True, non_zero=True, seed=None, equal=True): 
		self.gen.generate_data(data_num=data_num, center=center, non_zero=non_zero, seed=seed, equal=equal)
		self.core_subgroup = self.gen.core_subgroup
		self.subgroup_type = self.gen.subgroup_type
		self.cores = self.gen.cores
		return self.gen.df
	def save_dataset(self,path="",prefix="test"):
		self.gen.save_dataset(path=path,prefix=prefix)

	def get_features_wo_sensitive(self): 
		return self.gen.get_features_wo_sensitive()
	def get_features_w_sensitive(self):
		return self.gen.get_features_w_sensitive()
	def get_groundtruth(self):
		return self.gen.get_groundtruth()
	def get_unfair_groundtruth(self):
		return self.gen.get_unfair_groundtruth()
	def get_sensitive(self):
		return self.gen.get_sensitive()
	def get_cores(self):
		return self.gen.cores
	def get_core_subgroups(self):
		return self.gen.core_subgroup
	def get_subgroup_type(self):
		return self.gen.subgroup_type
class densityDataGen:
	...
	def __init__(self, dim=2, clunum=2, clu_ratios=None, core_num=10, min_ratio=0, ratio_noise=0, domain_size=20,
				 ...
				 safety=True, con_dens_factors=False, con_radius=2, con_step=2,  star=0, square=False,
				 random_start=False, distribution=None, pos=None, con_distribution=None , g:int=2, distr = [[0.55,0.45], [0.45,0.55]], gap = 2):
		"""
		...
		self.artificial_cores = clunum * g * gap
		if isinstance(core_num, list):
			self.core_num = [ 
				x + (g*gap) for x in core_num
			]
		else:	
			self.core_num = core_num + self.artificial_cores
	...
	def generate_cluster(self, cluid):
		...
		self.remove_subgroup_edges(cluid, gap = self.gap)


	def remove_subgroup_edges(self, cluid, gap=2):
		cores = self.cores[cluid]
		total = len(cores)
		group_num = self.subgroups
		usable = total - gap*(group_num-1)

		if usable < group_num:
			cores = cores[:-gap*self.g]
			self.cores[cluid] = np.array(cores)
		...
		base = usable // group_num
		remainder = usable % group_num
		sizes = []
		for i in range(group_num):
			# mixed group gets remainder
			if i == group_num-1:
		...
		for subgroup_id, size in enumerate(sizes):
			start = cursor
			end = cursor + size
			keep_indices.extend(
				range(start,end)
			)
			subgroup_labels.extend(
				[subgroup_id+self.subgroups*cluid]*size
			)
			# skip the edge/gap cores
			cursor = end + gap

		self.cores[cluid] = np.array(
			cores[keep_indices]
		)
		self.core_subgroup[cluid] = subgroup_labels
...
# --
	...
	def assign_sensitive_attributes_w_distr(
		self,
		subgroup_ids,
		cluster_ids,
		g,
		...
		"""
		Assign sensitive attributes to density-connected subgroups.

		Parameters
		----------
		subgroup_ids : array-like
			Density-connected subgroup labels.
		...
		-------
		sens : ndarray
			Sensitive attribute assignment.

		subgroup_type : dict
			Mapping subgroup_id -> subgroup behavior type.
		"""

		rng = np.random.default_rng(seed)

		subgroup_ids = np.asarray(subgroup_ids).astype(int)
		cluster_ids = np.asarray(cluster_ids).astype(int)

		sens = np.zeros(len(subgroup_ids), dtype=int)
		#g = len(distr)
		#print("distr : ", distr)
		distr = np.array(distr)
		mixed_distr = distr.mean(axis=0)
		subgroup_type = {}
		unique_clusters = np.unique(cluster_ids)

		for cluster in unique_clusters:
			cluster_mask = cluster_ids == cluster
			cluster_subgroups = np.unique(subgroup_ids[cluster_mask])
			#print("Cluster Subgroups of cluster ", cluster, " : ", cluster_subgroups)
			# ignore noise subgroup
			cluster_subgroups = cluster_subgroups[cluster_subgroups != -1]
			# fewer than g+1 subgroups -> all become mixed 
			if len(cluster_subgroups) < (g + 1):

				for sg in cluster_subgroups:
					subgroup_type[sg] = "mixed"

			# else assign dominant + mixed subgroup pattern
			else:
				for i, sg in enumerate(cluster_subgroups):
					if i % (g + 1) == g:
						subgroup_type[sg] = "mixed"
					else:
						subgroup_type[sg] = i % g
		# noise subgroup
		subgroup_type[-1] = "mixed"

		# sample sensitive attributes
		for i, sg in enumerate(subgroup_ids):
			t = subgroup_type[sg]
			# mixed subgroup follows global distribution
			if t == "mixed":
				probs = mixed_distr
			# else follows user defined distr
			else:
				probs = distr[t]
			sens[i] = rng.choice(g, p=probs)
 
		return sens, subgroup_type
	def assign_sensitive_attributes_w_exact_distr(
		self,
		subgroup_ids,
		cluster_ids,
		g,
		...
		"""
		Assign sensitive attributes deterministically according to the desired
		distributions (up to rounding), instead of sampling.

		Each subgroup receives the exact number of sensitive attributes implied
		by its distribution and size.
		...
		for sg in unique_subgroups:
			idx = np.where(subgroup_ids == sg)[0]
			n = len(idx)
			if n == 0:
				continue
			t = subgroup_type[sg]
	...
	def tooclose(self, pos, label, exclude=None, noise=False, current_subgroup=None):
		"""
	...
	def tooclose_pair(self, pos, label, partner, throwaway1,throwaway2):
		"""
...
# ----------------
	def generate_cluster_pos(self, start_pos, cluid):
		...
		if cluid not in self.core_subgroup:
			self.core_subgroup[cluid] = []
		g = self.subgroups
		self.core_subgroup[cluid].append(0)
		...
		for i in range(core_num - 1):
			...
			while (retry_last or retry_some):
				...
				# -- MOD HERE
				core_idx = len(self.cores[cluid])   # index candidate would get
				g = self.subgroups
				current_subgroup = min(core_idx * g // core_num, g - 1)

				# --
				if (self.tooclose(pos, cluid, current_subgroup=current_subgroup)): 
					if (tries_some >= self.max_retry):
						return False
					elif (tries_last >= self.max_retry):
						retry_some = True
					...
					core_idx = len(self.cores[cluid]) - 1
					g = self.subgroups

					subgroup_id = min(core_idx * g // core_num, g - 1)
					self.core_subgroup[cluid].append(subgroup_id)
	...
	def generate_data(self, data_num, center=True, non_zero=True, seed=None, equal=True):
		...
		maxs = []
		data = []
		X,y_cluster,y_subgroup = [],[],[]
	#	print(" CLU RATIOS ", self.clu_ratios)
		if seed is not None:
			set_data_seed(seed)
		else:
		...
		for cluid in self.cores.keys():
			...
			# print(cluid)
			clu_data_num = int(clu_ratio * data_num)
			testsum += clu_data_num

			if equal and clu_core_num != 0: # FIX case where cluster has 0 cores but equal is true
			...
			for coreid in range(len(self.cores[cluid])):
				...
				#core_data_num = max(5, core_data_num)
				if core_data_num == 0 and non_zero:
					core_data_num = 1
				if center and core_data_num > 0:

					core_data_num -= 1
					subgroup = self.core_subgroup[cluid][coreid]

					data.append(core.copy().tolist() + [cluid,subgroup])
					X.append(core.copy())
					y_cluster.append(cluid)
					y_subgroup.append(subgroup)
		...
		total_assigned = 0
		for cluid in self.cores:
			if cluid >= 0:
				if cluid == 0:
					ratio = self.clu_ratios[0]
				else:
					ratio = self.clu_ratios[cluid] - self.clu_ratios[cluid-1]
				total_assigned += int(ratio * data_num)


		if data_num - len(X) > 0 and self.ratio_noise == 0:
			clus = np_data_generator.choice(self.clunum, data_num - len(X), replace=True)
			for cluid in clus:
				clu_core_num = len(self.cores[cluid])
				dist = self.distributions[cluid]
				cluradius = self.r_sphere * self.dens_factors[cluid]
				coreid = np_data_generator.choice(clu_core_num,1)[0]
				core = self.cores[cluid][coreid]
				data_new = self.generate_distribution(core, cluradius, 1, dist, cluid)
				#print(cluid, coreid[0])
				subgroup = self.core_subgroup[cluid][coreid]
				for row in data_new: 
					X.append(row[:-1])
					y_cluster.append(cluid)
		...
		noisenum = max(round(data_num * self.ratio_noise), data_num - len(X))
		...
		sens, subgroup_type = self.assign_sensitive_attributes_w_exact_distr(
			y_subgroup,
			y_cluster,
			self.g,
			distr=self.distr,
		...
		idx_structured = np.where(np.array(y_cluster) != -1)[0]
		idx_noise = np.where(np.array(y_cluster) == -1)[0]
		target_structured = int(data_num * (1 - self.ratio_noise))
		target_noise = data_num - target_structured
		idx_structured_sampled = np_data_generator.choice(
			idx_structured,
			size=min(target_structured, len(idx_structured)),
			replace=False
			)

		idx_noise_sampled = np_data_generator.choice(
			idx_noise,
			size=min(target_noise, len(idx_noise)),
			replace=False
			)
		idx = np.concatenate([idx_structured_sampled, idx_noise_sampled])
		np_data_generator.shuffle(idx)
		X = X[idx]
		y_cluster = y_cluster[idx]
		y_subgroup = y_subgroup[idx]
		sens = sens[idx]
		...
		self.df = pd.DataFrame({
			**feature_cols,
			"groundtruth": y_cluster,
			"subgroup_id": y_subgroup,
	...
	def save_dataset(self,path="",prefix="test"):
		import json
		self.df.to_csv(f"{path}{prefix}_data.csv",index=False)
		metadata= {
			"seed":self.seed,
			"clunum":self.clunum,
			"dim":self.dim,
			...
			"distr":self.distr,
			"subgroup_type" : {
				str(k):v for k,v in self.subgroup_type.items()
			}
		}
		with open(
			f"{prefix}_metadata.json", "w"
		) as f:
